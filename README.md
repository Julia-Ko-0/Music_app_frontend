...
Помоги "донастроить" проект, настрой роуты в папке роутс и помоги отрисовать экраны избранное и домой, для этого создай отдельно компонент с меню AppMenu в котором две кнопки, которые и будут переключать между этих компонентов и меню создаеться через mainItems :...=useMemo (внутри находяться элементы меню с роутыми которые ниже в return будут отрисовываться ) переключение происходит за счет роутов чтобы отслеживать состояние в будующем, соблюдай архитектуру проекта Правильность названий папок и тдs
